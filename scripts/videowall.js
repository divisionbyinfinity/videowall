...
// --------------------------
// New helper: Rotation time parser.
// For rotation values (beginRotation/endRotation), we always use today’s date (or the given date)
	...
	try {
		...
		if (
			...
			if (timeRemaining > 0 && timeRemaining <= launchWaitWindow) {
				...
				// Copy the original launch value so that displaySlide uses it for the scheduled start div.
				launchWaitSlideData.launch = mainSlideData.launch;
		...
		// Optional beginRotation / endRotation checks.
		// These determine whether the slide is included in the normal rotation.
